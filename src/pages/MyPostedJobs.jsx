...
const MyPostedJobs = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      const { data } = await axios.get(
        `${import.meta.env.VITE_URL}/getAllJobs?email=${user?.email}`
      );
      setAddedJob(data);
    };
    fetchJobs();
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const filter = addedJob.filter((job) => job._id !== id);
        setAddedJob(filter);
  ...
  return (
    <section className="w-11/12 px-4 mx-auto pt-12">
                ...
                <tbody className="bg-white divide-y divide-gray-200 ">
                  {addedJob?.map((job) => (
                    <tr key={job._id}>
                      ...
                      <td className="px-4 py-4 text-sm text-gray-500  whitespace-nowrap">
                        {job?.deadline && format(new Date(job?.deadline), "P")}
                      </td>
